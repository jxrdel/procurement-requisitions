...
@section('title')
    <title>Cheque Dispatch Requisitions | Requisitions</title>
@endsection
    ...
    $(document).ready(function() {
        $('#myTable').DataTable({
            ...
            "columns": [
                ...
                {
                    data: 'ar_completed',
                    name: 'accounts_requisitions.is_completed',
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function (data, type, row) {
                        return ' <div style="text-align:center"><a class="btn btn-primary" href="/accounts_requisitions/view/' + data.ar_id + '" >View</a> </div>';
                    }
                },
                { 
                    data: 'ar_created_at', 
                    name: 'accounts_requisitions.created_at', 
    ...
    $('.btn-check').change(function() { //Table Filter
              ...
              switch(selectedOption) {
                  case 'btn-in-progress':
                      $('#myTable').DataTable().ajax.url('{{ route("getinprogressaccounts_requisitions") }}').load();
                      break;
                  case 'btn-completed':
                      $('#myTable').DataTable().ajax.url('{{ route("getcompletedaccounts_requisitions") }}').load();
                      break;
                  case 'btn-all':
                      $('#myTable').DataTable().ajax.url('{{ route("getaccounts_requisitions") }}').load();
                      break;
              }
      });
